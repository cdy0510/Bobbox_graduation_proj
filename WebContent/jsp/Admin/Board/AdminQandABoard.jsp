...
<body>
	...
	<div id="board_area">	
		<table>
			...
			<tr id="table-content">
				...
				<td id="board-title"><a href="AdminQandAboardForm.jsp?QandA_board_num=<%=QandA_board.getQandA_board_num()%>&pageNum=<%=currentPage%>">
    ...
    if (count > 0) {
        ...
        if (startPage > 10) { %>
          <a href="AdminQandABoard.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
<%      }
        
        for (int i = startPage ; i <= endPage ; i++) {  %>
           <a href="AdminQandABoard.jsp?pageNum=<%= i %>">[<%= i %>]</a>
<%      }
        
        if (endPage < pageCount) {  %>
        <a href="AdminQandABoard.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
<%
        }
    }
%>
